...
def _is_audio_model(model_name):
    ...
    return model_name.startswith("HTSAT") or model_name.startswith("whisper")
...
def profile_model(model_name, batch_size=1, profiler='torch'):
    ...
    if is_audio:
        ...
        results['image_size'] = 0  # not applicable for audio
        ...
        device = next(model.parameters()).device
        audio_input = {
            "waveform": torch.randn(batch_size, clip_samples, device=device),
